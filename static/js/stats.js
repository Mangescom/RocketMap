            ...
            if (pkmnCount[i] && pkmnCount[i].Count > 0) {
                pokeCounts.push(
                    [
                        ...
                        '<a href=\'http://pokemon.gameinfo.io/en/pokemon/' + pkmnCount[i].ID + '\' target=\'_blank\' title=\'Pokédex Megnyitása\' style=\'color: black;\'>' + pkmnCount[i].Name + '</a>',
                        pkmnCount[i].Count,
        ...
        for (i = 0; i < arenaCount.length; i++) {
            if (arenaCount[i] > 0) {
                if (i === 1) {
                    ...
                    arenaListString += '<tr><td><img class="arenaListString" src="static/images/gym/Uncontested.png" /></td><td>' + 'Üres' + '</td><td>' + arenaCount[i] + '</td><td>' + Math.round(arenaCount[i] * 100 / arenaTotal * 10) / 10 + '%</td></tr>'
                }
            }
        }
    ...
    if (Store.get('showPokestops')) {
        ...
        for (i = 0; i < pokestopCount.length; i++) {
            if (pokestopCount[i] > 0) {
                if (i === 0) {
                    pokestopListString += '<tr><td><img class="pokestopListString" src="static/images/pokestop/Pokestop.png" /></td><td>' + 'Lure nélküli' + '</td><td>' + pokestopCount[i] + '</td><td>' + Math.round(pokestopCount[i] * 100 / pokestopTotal * 10) / 10 + '%</td></tr>'
                } else if (i === 1) {
                    pokestopListString += '<tr><td><img class="pokestopListString" src="static/images/pokestop/PokestopLured.png" /></td><td>' + 'Lure-ozott' + '</td><td>' + pokestopCount[i] + '</td><td>' + Math.round(pokestopCount[i] * 100 / pokestopTotal * 10) / 10 + '%</td></tr>'
                }
            }
        }
